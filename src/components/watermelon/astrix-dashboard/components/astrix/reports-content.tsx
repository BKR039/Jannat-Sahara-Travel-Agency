...
export function ReportsContent() {
  ...
  const typeFilterLabel =
    reportTypeFilterOptions.find((option) => option.value === typeFilter)?.label ??
    "All Report Type";
  const statusFilterLabel =
    reportStatusFilterOptions.find((option) => option.value === statusFilter)?.label ??
    "All Status";
  const dateRangeLabel =
    dateRangeOptions.find((option) => option.value === dateRange)?.label ?? "Last 7 Days";
  ...
  const typeFilterControl = (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" collisionPadding={16} className="astrix-dashboard min-w-44">
        <DropdownMenuRadioGroup
          value={typeFilter}
  ...
  const statusFilterControl = (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" collisionPadding={16} className="astrix-dashboard min-w-40">
        <DropdownMenuRadioGroup
          value={statusFilter}
          onValueChange={(value) => setStatusFilter(value as ReportStatusFilter)}
        >
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {reportTypes.map((type) => {
            ...
            return (
              <div key={type.id} className="flex flex-col gap-8 rounded-xl border p-4 pb-3.5">
                ...
                <div className="flex flex-1 flex-col gap-4">
                  ...
                  <Button type="button" variant="secondary" className="h-10 w-full gap-2">
                    Generate Report
        ...
        <div className="flex flex-col gap-4 md:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <ChevronDownIcon
                className={cn("size-5 shrink-0 transition-transform", filtersOpen && "rotate-180")}
              />
            </Button>
          </div>
...
function ReportRowItem({ report }: { report: GeneratedReport }) {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <Button type="button" variant="ghost" size="lg" disabled={!canDownload} className="gap-2">
          Download
