...
import { classificationQueue, type ClassificationRow, type ClassificationStatus } from "../../data";
...
export function ClassificationContent() {
  ...
  const counts = useMemo(() => {
    return {
      ...
      processing: classificationQueue.filter((row) => row.status === "processing").length,
      "needs-review": classificationQueue.filter((row) => row.status === "needs-review").length,
      approved: classificationQueue.filter((row) => row.status === "approved").length,
    };
  }, []);
              ...
              >
                ...
                >
                  {filters.map((filter) => (
                    ...
                    >
                      {filter.label}
                      <span className="ml-auto text-xs tabular-nums">{counts[filter.value]}</span>
                    </DropdownMenuRadioItem>
                  ))}
                </DropdownMenuRadioGroup>
              </DropdownMenuContent>
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        isActive ? "bg-background text-primary" : "bg-background text-foreground",
                      )}
                    >
                      {counts[filter.value]}
                    </span>
